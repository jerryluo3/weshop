<?php
class Discountcategory_model extends CI_Model {
	 ...
	 public function get_category($slug = FALSE)
    {
		...
		$cates = $this->db->order_by('cat_id','ASC')->get('discountcategory')->result_array();
		//echo $this->db->last_query();
		$data['cates'] = $this->discountcate->resetCategoryData($cates,0,$_SESSION['user_purview']);
...
//		}
		...
		$cates = $this->db->get("discountcategory")->result_array();
		$catesoption = $this->discountcate->resetCategoryData($cates,$optionlist,$purview,$selectedid);
		return $catesoption;
	}
...
//	}
	...
	public function getonecate($id){
		if(empty($id)) return "";		
		return $this->db->where(array('cat_id'=>$id))->get('discountcategory')->row_array();
	}
	...
	public function update()
	{
		...
		if($fid > 0){
			$frow = $this->db->where(array('cat_id'=>$fid))->get('discountcategory')->row_array();
		...
		if($action == "mod" && $cat_id > 0){
			//判断有没有修改栏目
			$ofrow = $this->db->where(array('cat_id'=>$cat_id))->get('discountcategory')->row_array();
			...
			$where = "cat_id=".$cat_id;
			$this->db->update('discountcategory', $data, $where);
			
					
		}else{
			$this->db->insert('discountcategory', $data);
	...
	public function del($ids){
		//删除
		if(empty($ids)) return "";
		$frow = $this->db->where(array('cat_id'=>$ids))->get('discountcategory')->row_array();
		if(isset($frow['cat_fid']) && $frow['cat_fid'] > 0){
			$this->updateCateSon($frow['cat_fid'],$ids,1);
		}
		$this->db->delete('discountcategory', array('cat_id' => $ids));
		$data['err'] = '';
		return $data;
	}
