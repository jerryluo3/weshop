...
class Category_model extends CI_Model {
	 ...
	 public function get_category($slug = FALSE)
    {
		...
		$cates = $this->db->order_by('cat_id','ASC')->get('category')->result_array();
...
//	}
	...
	public function getonecate($id){
		if(empty($id)) return "";		
		return $this->db->where(array('cat_id'=>$id))->get('category')->row_array();
	}
	...
	public function del($ids){
		//删除
		if(empty($ids)) return "";
		$frow = $this->db->where(array('cat_id'=>$ids))->get('category')->row_array();
		if(isset($frow['cat_fid']) && $frow['cat_fid'] > 0){
			$this->updateCateSon($frow['cat_fid'],$ids,1);
		}
		$this->db->delete('category', array('cat_id' => $ids));
		$data['err'] = '';
		return $data;
	}
