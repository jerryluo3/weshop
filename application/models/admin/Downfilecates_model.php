<?php
class Downfilecates_model extends CI_Model {
    ...
    {		
		...
		$cates = $this->db->order_by('cat_id','ASC')->get('downfile_category')->result_array();
		//echo $this->db->last_query();
		$data['cates'] = $this->cate->resetCategoryData($cates,0,$_SESSION['user_purview']);
...
//	}
	...
	public function getonecate($id){
		if(empty($id)) return "";		
		return $this->db->where(array('cat_id'=>$id))->get('downfile_category')->row_array();
	}
	...
	public function update()
	{
		...
		if($fid > 0){
			$frow = $this->db->where(array('cat_id'=>$fid))->get('downfile_category')->row_array();
		...
		if($action == "mod" && $cat_id > 0){
			//判断有没有修改栏目
			$ofrow = $this->db->where(array('cat_id'=>$cat_id))->get('downfile_category')->row_array();
			...
			$where = "cat_id=".$cat_id;
			$this->db->update('downfile_category', $data, $where);
			
					
		}else{
			$this->db->insert('downfile_category', $data);
	...
	public function del($ids){
		//删除
		if(empty($ids)) return "";
		$frow = $this->db->where(array('cat_id'=>$ids))->get('downfile_category')->row_array();
		if(isset($frow['cat_fid']) && $frow['cat_fid'] > 0){
			$this->updateCateSon($frow['cat_fid'],$ids,1);
		}
		$this->db->delete('downfile_category', array('cat_id' => $ids));
		$data['err'] = '';
		return $data;
	}
	...
	public function makeFclass(){
		...
		$arr = $this->db->get('downfile_category')->result_array();
