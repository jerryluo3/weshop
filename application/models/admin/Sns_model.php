...
class Sns_model extends CI_Model {
	...
	public function getSnsList()
	{
		...
		$total_nums = $this->db->count_all_results('sns');
		$page_size = 20;
		...
		$data['sns_list'] = $this->db->order_by('id','DESC')->limit($page_size,$offset)->get('sns')->result_array();
	...
	public function del()
	{
		...
		if($id > 0){
			$this->db->delete('sns', array('id' => $id));
			$data['err'] = '';
		}else{
			$data['err'] = '参数出错';	
		}
