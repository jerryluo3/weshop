<?php
class Linkscates_model extends CI_Model {
	
	
	/******广告类别******/
	
	public function getLinkscatesList()
	{
	
		$total_nums = $this->db->count_all_results('links_category');
		$page_size = 20;
		...
		$data['linkscates_list'] = $this->db->order_by('cat_id DESC')->limit($page_size,$offset)->get('links_category')->result_array();
		//分页
	...
	public function getonelinkscates(){
		$cat_id = $this->uri->segment(4);
		if(!$cat_id) return '';
		return $this->db->where(array('cat_id'=>$cat_id))->get('links_category')->row_array();
	}
	...
	public function update()
	{
		...
		if($action == 'add'){
			$this->db->insert('links_category',$data);
		}
		if($action == 'mod'){
			$this->db->update('links_category',$data,array('cat_id'=>$cat_id));	
		}
	...
	public function del()
	{
		$cat_id = $this->uri->segment(4);
		if($cat_id > 0){		
			$this->db->delete('links_category', array('cat_id' => $cat_id));
			$data['err'] = '';
		}else{
			$data['err'] = '参数出错';	
		}
	...
	public function pagelist($total_nums,$page_size){
	    ...
	    $config = array(
				'base_url'       => base_url(admin_url().'links/'),
